...
class AudioEqualizer:
    ...
    def process_and_play(self):
        ...
        signal = self.original_audio
        N_total = len(signal)

        processed = np.zeros(N_total, dtype=np.float32)

        total_blocks = (N_total + block_size - 1) // block_size
        ...
        for start in range(0, N_total, block_size):
            block_counter += 1
            # print(f"Processing block {block_counter}/{total_blocks}")
            ...
            discrete_block = DiscreteSignal(block)
            spectrum = analyzer.compute_dft(discrete_block)
            ...
            for i in range(5):
                b_start = i * band_size
                b_end = (i + 1) * band_size if i < 4 else N
                spectrum[b_start:b_end] *= gains[i]

            processed_block = analyzer.compute_idft(spectrum).real
            processed[start:end] = processed_block[:min(block_size, N_total - start)]

        max_val = np.max(np.abs(processed))
        if max_val > 1.0:
            processed /= max_val

        self.processed_audio = processed.astype(np.float32)
