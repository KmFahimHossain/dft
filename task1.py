...
class DoodlingApp:
    ...
    def run_transform(self):
        ...
        if self.use_fft.get():
            ...
            new_length = 2 ** power
            signal = signal.interpolate(new_length)
            analyzer = FastFourierTransform()
        else:
            analyzer = DFTAnalyzer()

        self.fourier_coeffs = analyzer.compute_dft(signal)
        self.N = len(self.fourier_coeffs)

        mean_point = np.mean(signal.data)
        self.path = []
    ...
    def update_frame(self):
        ...
        N = self.N
        x = self.center_offset.real
        y = self.center_offset.imag

        coeffs = list(enumerate(self.fourier_coeffs))
        coeffs.sort(key=lambda pair: np.abs(pair[1]), reverse=True)
        for k, Xk in coeffs:
            prev_x, prev_y = x, y
            radius = np.abs(Xk) / N
            angle = (2 * np.pi * k * t / N) + np.angle(Xk)
